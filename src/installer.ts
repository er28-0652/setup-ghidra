...
async function getGhidraVersionInfo(): Promise<Dict> {
  // get release page
  let topPageHTML = await (
    await http.get(GHIDRA_BASE_URL)
  ).readBody();
...
async function extractGhidraArchive(src: string, dst: string): Promise<string> {
  ...
  if (stats.isFile()) {
    ...
    const ghidraDirectory = path.join(
      dst,
      fs.readdirSync(dst)[0]
    );
...
export async function installGhidra(version: string = ''): Promise<void>{
  ...
  if (toolPath) {
    ...
    console.log(`Version: ${ghidraVersionInfo.version}, Archive: ${ghidraVersionInfo.archive}`);
    let savedPath = await tc.downloadTool(GHIDRA_BASE_URL + ghidraVersionInfo.archive);
